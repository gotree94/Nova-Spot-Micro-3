...
class STLViewer(QMainWindow):
    ...
    def _load_stl(self, filepath: str):
        ...
        # 법선 계산 - 30도 이상 각도 차이나는 면은 날카로운 엣지로 분리
        ...
        # 엣지 라인 오버레이 (Creality 스타일 엣지 강조)
        edge_mapper = vtk.vtkPolyDataMapper()
        edge_mapper.SetInputConnection(normals.GetOutputPort())

        self.edge_actor = vtk.vtkActor()
        self.edge_actor.SetMapper(edge_mapper)
        ep = self.edge_actor.GetProperty()
        ep.SetRepresentationToWireframe()
        ep.SetColor(0.0, 0.0, 0.0)
        ep.SetOpacity(0.08)
        ep.SetLineWidth(0.5)
    ...
    def _toggle_wireframe(self, checked: bool):
        if self.vtk_actor:
            saved_color = self._color_map.get(self.current_file, (0.3, 0.6, 1.0))
            if checked:
                self.vtk_actor.GetProperty().SetRepresentationToWireframe()
                self.vtk_actor.GetProperty().SetColor(*saved_color)
                if self.edge_actor:
